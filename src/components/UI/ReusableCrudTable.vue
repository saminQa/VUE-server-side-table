              ...
              >
                <v-row>
                  ...
                  <v-col cols="12" sm="9" md="3" lg="3" xl="3" class="mt-lg-3 mt-md-3 tabulka-search-div">
                    <v-text-field
                      v-model="big_search"
                      label="Search Addressee, Type or Content"
...
export default {
  ...
  props: { // check the example
    ...
    columnFilters: { // filters that appear under the table header in specific column. "[ {column: 'string from headers[key]',type: 'select/text',queryParam: 'query param from B-E', items: items to inhabit the select menu with, title: 'if returned data else as title and value', value: 'if returned data else as title and value' },example:{ column: 'string', type: 'text', queryParam: 'search_ulica' },]"
      type: Array,
      default: () => [],
    },
    fetchItems: {
  ...
  methods: {
    async fetchData() {
      ...
      const params = {};
      try {
        const { items = [], total = 0 } = await this.fetchItems({
    ...
    resetFilters() {
      this.filters = {};
      this.big_search='',
      this.showSuccess('Filtering cleared.');
      this.applyFilters();
    ...
    async validateAndSave() {
      try {
        ...
        if (valid) {
          ...
          this.showError({ message: 'Please fill in all required fields.' });
        }
      } catch (error) {
        console.error('Validation error:', error);
        this.showError({ message: 'Error during validation.' });
      }
    },
    ...
    async save() {
      try {
        if (this.editedIndex > -1) {
          ...
          this.showSuccess('Record was updated successfully.');
        } else {
          // Create
          await this.createItem(this.editedItem);
          this.showSuccess('Record was created successfully.');
        }
    ...
    handleRowClick(item) {
      if (item) {
        ...
        console.error(
          'No records found. Please check your data source or fetch data from the server.',
        );
      }
    },
    ...
    async deleteItemConfirm() {
      try {
        ...
        this.showSuccess('Record was deleted successfully.');
        await this.fetchData(); // Refresh the table
        this.closeDelete();
        this.close();
      } catch (error) {
